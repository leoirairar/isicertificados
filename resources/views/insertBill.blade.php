...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header" style="font-weight: bold">Registrar factura de empleado</div>
                @if (session('error'))
                    <div class="alert alert-danger">{{ session('error') }}</div>
                @endif
                <div class="card-body">
                    @if (isset($employeeEnrollment->employee->user))
                    <div class="alert alert-warning" role="alert">
                            Aún no hay usuarios matriculados a ningun curso ¡¡
                    </div>
                    @else
                    <form method="POST" action="{{ route('insertBill') }}">
                        @csrf 
                        <div class="form-group row">
                            <label for="payment" class="col-md-4 col-form-label text-md-right">{{ __('Valor de curso') }}</label>

                            <div class="col-md-6">
                                <input id="payment" type="number" class="form-control @error('payment') is-invalid @enderror" name="payment" value="{{ old('payment') }}" required autocomplete="payment" autofocus> @error('payment')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="billSerial" type="text" class="form-control @error('billSerial') is-invalid @enderror" name="billSerial" value="{{ old('billSerial') }}" required autocomplete="billSerial" autofocus> @error('billSerial')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span> @enderror
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="paymentDay" type="date" class="form-control @error('paymentDay') is-invalid @enderror" name="paymentDay" value="{{ old('paymentDay') }}" required autocomplete="paymentDay" autofocus> @error('paymentDay')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span> @enderror
                            </div>
                                    ...
                                    <select class="form-control @error('enrollmentId') is-invalid @enderror enrollmentId" id="enrollmentId" name="enrollmentId"  required autocomplete="current-enrollmentId">
                                        <option value="">Selecciona una opción</option>
                                            ...
                                            @foreach ($employeesEnrollment as $employeeEnrollment)
                                                <option value="{{ $employeeEnrollment->id }}">{{ $employeeEnrollment->employee->user->name.'-'.$employeeEnrollment->courseProgramming->course->name}}</option> 
                                            @endforeach  
